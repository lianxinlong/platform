...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      //添加行政区矢量数据
      const arcgisProvider1 = new Cesium.ArcGisMapServerImageryProvider({
        url: "http://192.168.80.169:6080/arcgis/rest/services/重庆行政区划黄色/MapServer",
      });
      viewer.imageryLayers.addImageryProvider(arcgisProvider1);

      //添加重庆市范围矢量数据
      const   arcgisProvider2 = new Cesium.ArcGisMapServerImageryProvider({
        url: "http://192.168.80.169:6080/arcgis/rest/services/重庆市/MapServer",
      });
      viewer.imageryLayers.addImageryProvider(arcgisProvider2);
